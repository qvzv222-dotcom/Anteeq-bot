...
async def access_control_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 2:
        ...
        help_text = (
            ...
            f"1.1. ⏰ Выдать мут: <i>мут, размут</i> (от ранга {access_control.get('1.1', 1)})\n"
            f"1.2. ⚠️ Выдать предупреждение: <i>пред, варн</i> (от ранга {access_control.get('1.3', 1)})\n"
            f"1.3. 🔨 Выдать бан: <i>бан, разбан</i> (от ранга {access_control.get('1.2', 3)})\n\n"
            "🟡 <b>РАЗДЕЛ 2: Система ников</b>\n"
            f"2.1. ✏️ Установить себе ник: <i>+ник</i> (от ранга {access_control.get('2.1', 0)})\n"
            f"2.2. 🗑️ Удалить себе ник: <i>-ник</i> (от ранга {access_control.get('2.1', 0)})\n"
            f"2.3. 📝 Установить ник участнику: <i>+ник другому</i> (от ранга {access_control.get('2.2', 2)})\n"
            f"2.4. ❌ Удалить ник участнику: <i>-ник другому</i> (от ранга {access_control.get('2.2', 2)})\n\n"
            "🟢 <b>РАЗДЕЛ 3: Информирование</b>\n"
            f"3.1. 📋 Узнать правила: <i>правила</i> (от ранга {access_control.get('3.1', 3)})\n"
            f"3.2. ✍️ Изменить правила: <i>+правила</i> (от ранга {access_control.get('3.1', 3)})\n"
            f"3.3. 👋 Сообщение приветствия: <i>приветствие</i> (от ранга {access_control.get('3.2', 3)})\n"
            f"3.4. 📢 Изменить приветствие: <i>+приветствие</i> (от ранга {access_control.get('3.2', 3)})\n"
            f"3.5. 👨‍💼 Список администраторов: <i>кто админ</i> (от ранга {access_control.get('3.1', 3)})\n\n"
            "🔵 <b>РАЗДЕЛ 4: Администраторские</b>\n"
            f"4.1. 🛡️ Доступ к командам: <i>дк</i> (от ранга {access_control.get('4', 4)})\n"
            f"4.2. 🔗 Разрешить ссылки: <i>дк ссылки [ранг]</i> (от ранга {link_posting_rank})\n\n"
            "🟣 <b>РАЗДЕЛ 5: Система вознаграждения</b>\n"
            f"5.1. 🏆 Выдача наград: <i>!наградить {{награда}}</i> (от ранга {award_giving_rank})\n"
            f"5.2. ✂️ Снятие наград: <i>!снять награды</i> (от ранга {award_giving_rank})\n"
            "5.3. 🎖️ Посмотреть награды: <i>Наградной список</i> (от ранга 0)\n"
            f"5.4. 🎯 Изменить ранг награждения: <i>дк награды [ранг]</i> (от ранга {access_control.get('4', 4)})"
        )

...
<b>1. Система наказаний:</b>
"""
        rank_names = {0: "Участник", 1: "Модератор", 2: "Наборщик", 3: "Заместитель", 4: "Глава", 5: "Альянс"}
        
        sections_desc = {
            "1.1": "1.1 - Варн (варн)",
            "1.2": "1.2 - Мут/Размут (мут, размут)",
            "1.3": "1.3 - Бан/Кик (бан, разбан, кик)",
            "1.4": "1.4 - Просмотр чужих варнов (преды [кол-во])",
            "1.5": "1.5 - Завещание (!завещание, -завещание)",
            "2.1": "2.1 - Ник для себя (+ник, -ник)",
            "2.2": "2.2 - Ник для других (+ник другому, -ник другому)",
            "3.1": "3.1 - Назначение рангов (назначить)",
            "3.2": "3.2 - Приветствие (+приветствие)",
            "3.3": "3.3 - Правила (+правила)",
            "3.4": "3.4 - Роли/Настройки",
            "3.5": "3.5 - Код чата (!код чата)",
            "3.6": "3.6 - История наказаний (история наказаний)",
            "3.7": "3.7 - Управление ДК (дк)",
            "3.8": "3.8 - Фильтр мата (+маты, -маты, !преды)",
            "4": "4 - Награды (!наградить, !снять награды)"
        }
        
        for section in sorted(access_control.keys()):
            rank = access_control[section]
            desc = sections_desc.get(section, section)
            rank_name = rank_names.get(rank, str(rank))
            info += f"\n{desc}: <b>{rank_name}</b>"
        
        info += "\n\n<b>Использование:</b> <code>дк [раздел] [ранг]</code>\nПример: <code>дк 1.1 2</code>"

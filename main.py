...
<b>🎁 ДОПОЛНИТЕЛЬНО</b>
  • <code>дк награды</code> - управление наградами
  • <code>!кодчата</code> - получить код чата
...
async def access_control_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 2:
        ...
        help_text = (
            ...
            f"1.1. ⏰ Выдать мут: <i>мут, размут</i> {rank_emoji[r_1_1]}\n"
            f"1.2. ⚠️ Выдать предупреждение: <i>пред, варн</i> {rank_emoji[r_1_3]}\n"
            f"1.3. 🔨 Выдать бан: <i>бан, разбан</i> {rank_emoji[r_1_2]}\n\n"

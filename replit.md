...
## Recent Changes
- **2025-12-01**: FIXED - Rank & Nickname & Access Control Persistence Bug ✅
  - **Критическая ошибка**: Функция `restore_creator_ranks()` восстанавливала только создателей, но НЕ все ранги, ники и доступы к командам
  - **Решение**: Переписана функция восстановления для сохранения:
    - ✅ ВСЕ ранги пользователей из таблицы admins
    - ✅ ВСЕ ники из таблицы nicks
    - ✅ ВСЕ доступы к командам (access_control JSON) из таблицы chats
  - **Улучшения**: Добавлено логирование:
    - При сохранении ранга: `💾 Сохранен ранг X для пользователя Y`
    - При сохранении ника: `💾 Сохранен ник 'nickname'`
    - При сохранении доступов: `💾 Сохранены доступы команд`
    - При восстановлении: детальный вывод всех восстановленных данных
  - **Проверка целостности БД**: Убедиться что все чаты из admins существуют в таблице chats
  - **Результат**: Ранги, ники И ДОСТУПЫ к командам теперь СОХРАНЯЮТСЯ правильно при перезапуске бота 🎯
...
- **2025-12-01**: Advanced Russian Punctuation & Comma Rules ✅
  ...
  - **Запятые в перечислениях БЕЗ союза**: Между подряд идущими обычными словами
    - Пример: "яблоки груши апельсины" → "Яблоки, груши, апельсины."
  - **Запятые со связующими союзами**: "и", "или", "либо"
    - Пример: "яблоки и груши и апельсины" → "Яблоки, и груши, и апельсины."
  - **Капитализация**: Первая буква в начале и после точки/вопроса/восклицания
  - **Служебные слова**: Полный список местоимений, предлогов, вспомогательных глаголов (быть, был, было, будут и т.д.)
  - **Простой подход**: Без pymorphy2 (Python 3.11 совместимость), чистая логика на списках слов
